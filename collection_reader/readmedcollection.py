...
def build_med_collection()-> List['document']:

  docs: List[document] = []
  texts: List[str] = [] 
  cran_file = Path("./test_collections/med/MED.ALL")
...
def read_med_query() -> defaultdict:
...
def read_med_rel() -> defaultdict:
  ...
  with open (str(relevants_file),'r')as rel_f:
    for line in rel_f:
      query ,_,doc_id, rel = [int(item) for item in line.split()]
